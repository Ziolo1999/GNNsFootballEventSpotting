...
class SoccerNetClips(Dataset):
    def __init__(self, path, split="train", args=None): 
        ...
        self.args = args
        self.tiny = args.tiny
        self.listGames = ["england_epl/2014-2015/2015-02-21 - 18-00 Chelsea 1 - 1 Burnley"]
        ...
        if self.args.calibration_field:
            self.radar_image = cv2.imread("config/radar.png", cv2.IMREAD_COLOR)
            if self.args.teacher:
                self.radar_image = cv2.imread("config/model-radar-mini.png", cv2.IMREAD_COLOR)
