...
class ContextualNetVLAD(nn.Module):
    def __init__(self, cluster_size, feature_size, add_batch_norm=True):
        super(ContextualNetVLAD, self).__init__()
        self.feature_size = feature_size
        self.cluster_size = cluster_size
        self.clusters = nn.Parameter((1/math.sqrt(feature_size))
                *th.randn(feature_size, cluster_size))
    ...
    def forward(self,x):
        ...
        a = assignment*self.clusters2

        x = x.view(-1, max_sample, self.feature_size)
        x = x.unsqueeze(-1)
        vlad = th.matmul(x,assignment)
        vlad = vlad - a
        # L2 intra norm
        vlad = F.normalize(vlad)
        
        # # flattening + L2 norm
        vlad = vlad.reshape(-1, max_sample, self.cluster_size*self.feature_size)
        vlad = F.normalize(vlad)
