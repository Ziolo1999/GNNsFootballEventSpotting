...
def main():
    seg_paths = [f"/project_antwerp/models/finetuned/finetuned_{ann}.pth.tar" for ann in event_enc.keys()]
    args = Args
    collate_fn = collateGCN
    list_anns = list(event_enc.keys())

    for i, ann in enumerate(list_anns):
        args.focused_annotation = ann
        args.sgementation_path = seg_paths[i]
        ...
        losses = trainer(train_loader, validate_loader,
                            ...
                            save_dir=f"/project_antwerp/models/finetuned/spotting_finetuned_{ann}.pth.tar", 
                            train_seg=False)

        with open(f'/project_antwerp/results/finetuned/spotting_finetuned_{ann}.pkl', 'wb') as file:
